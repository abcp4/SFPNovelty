...
def rhea_act(env,it=10,pop_evolutions=10,pop_num=10,rollout_limit=20,mutation_prob = 0.1,use_shift_buffer=False,do_ns=False,run_type = 0,pop_tournment=3):
    ...
    re = RollingHorizonEvolutionaryAlgorithm(rollout_limit,env,mutation_prob,
                                            ...
                                            do_ns=do_ns,pop_tournment=pop_tournment)
    if(run_type==1):
        return re.run()
    if(run_type==2):
        return re.run2(it=it)
    if(run_type==3):
        return re.run3(it=it,pop_evolutions=pop_evolutions)
...
def cmcts_act(env,playouts,depth,do_ns=False,sel_type = 'ucb',update_type='default'):
    from custom_mcts import Runner
    return Runner(playouts=playouts, max_depth=depth,do_ns=do_ns,sel_type=sel_type,update_type=update_type).run(env)
