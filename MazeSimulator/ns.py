...
class NoveltySearch():
    ...
    def get_hamming_behavior(self,behv,episode_behavior,act,done):
        if(done==False):
            feature_vec=np.zeros(250)
            feature_vec[int(behv[0])//10]=1
            feature_vec[int(behv[1])//10 +100]=1
            feature_vec[int(act[0])+200]=1
            feature_vec[int(act[1])+210]=1
            feature_vec[int(act[2])+220]=1
            print('behv: ',behv,', act: ',act)
        ...
        if(done==True):
            ...
            if min_score>0:
                if(behv_score>self.min_score):
                    #if(str(behv) not in self.behavior_cache):
                    #    #print('new behavior: ',behv)
